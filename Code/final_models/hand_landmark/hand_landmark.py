...
class HandLandmark(object):
    ...
    def __call__(
        ...
        """
        print(f"[DEBUG] inference_images shape: {inference_images.shape if hasattr(inference_images, 'shape') else 'N/A'}")
        print(f"[DEBUG] Number of resized_images: {len(resized_images)}")
        print(f"[DEBUG] resize_scales_224x224: {resize_scales_224x224}")
        print(f"[DEBUG] half_pad_sizes_224x224: {half_pad_sizes_224x224}")
        """
        ...
        """
        print(f"[DEBUG] xyz_x21s shape: {xyz_x21s.shape if hasattr(xyz_x21s, 'shape') else 'N/A'}")
        print(f"[DEBUG] hand_scores: {hand_scores}")
        print(f"[DEBUG] left_hand_0_or_right_hand_1s: {left_hand_0_or_right_hand_1s}")
        """
        ...
        """
        print(f"[DEBUG] hand_landmarks: {hand_landmarks}")
        print(f"[DEBUG] rotated_image_size_leftrights: {rotated_image_size_leftrights}")
        """
    ...
    def __postprocess(
        ...
        hand_landmarks = np.asarray([], dtype=np.float32)
        extracted_hands = []
        ...
        for resized_image, resize_scale_224x224, half_pad_size_224x224, rect, xyz_x21, left_hand_0_or_right_hand_1 in \
            ...
            keypoints = []
            for i, (x,y) in enumerate(rescaled_xy):
                coord_arr = np.array([
                    [x, y, 1],  # Left-Top
                ])
                ...
                x_ls = new_coord[0]
                y_ls = new_coord[1]
                keypoints.append([int(x_ls), int(y_ls), z_values[i]])
            ...
            hand_landmarks = np.asarray(keypoints, dtype=np.float32).reshape(-1,3)
            hand_landmarks[:, 0:2] = hand_landmarks[:, 0:2].astype(np.int32)
        ...
        valid_hands, valid_sizes = [], []
        for h, s in zip(extracted_hands, rotated_image_size_leftrights):
            h_arr = np.asarray(h)
            if h_arr.shape == (21, 3):
                valid_hands.append(h_arr)#h_arr.astype(np.int32))
                valid_sizes.append(s)

        if not valid_hands:            # nothing usable this frame
            return [], []

        return valid_hands, valid_sizes
